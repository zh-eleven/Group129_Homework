...
void SM3_last(SM3* md, unsigned char hash[])
{
	...
	compress(md);
	memcpy(hash, md->state, 32);
	BigEndian(hash, 256 / 8, hash);
}


void init1(SM3* md,unsigned int V[])
{
	md->state[0] = V[0];
	md->state[1] = V[1];
	md->state[2] = V[2];
...
void SM3_last1(SM3* md, unsigned char hash[],int padlen)
{
	int i;
	unsigned char tmp = 0;
	md->len += md->curlen << 3;
	...
	md->buf[63] = padlen & 0xff;
	md->buf[62] = (padlen >> 8) & 0xff;
	md->buf[61] = (padlen >> 16) & 0xff;
	md->buf[60] = (padlen >> 24) & 0xff;
	compress(md);
	memcpy(hash, md->state, 32);
	BigEndian(hash, 256 / 8, hash);
}
...
void SM3_256_len(unsigned char buf[], int len, unsigned char hash[],unsigned int temp[],int padlen)
{
	SM3 md;
	init1(&md,temp);
	SM3_first(&md, buf, len);
	SM3_last1(&md, hash,padlen);
}
...
int main()
{
	...
	unsigned char
		...
		0x61,0x62,0x63,0x64,0x61,0x62,0x63,0x64,0x61,0x62,0x63,0x64,0x61,0x62,0x63,0x64 };
	int MsgLen2 = 64;
	lenAttack(Msg2, MsgLen2, Msg1, MsgLen1);
}